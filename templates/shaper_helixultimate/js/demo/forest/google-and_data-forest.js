const jsonFilePath = '/templates/shaper_helixultimate/json/forest/forest_data.json';
...
*/
        ...
        const dayWithZero = dateParts[2]; // Получаем день с нулем в начале (напр. 01, 07 и т.д.)
const day = parseInt(dateParts[2], 10); // Преобразуем в число и удаляем 0 (напр. 1, 7)
    ...
    const timeArParts = data.time.guestsArrival.split(':');
       ...
       document.getElementById('wedding-date').textContent = `${day}.${monthNumberWithZero}.${year}`;
       document.getElementById('guests-arrival').textContent = `Сбор гостей в  ${data.time.guestsArrival}`;
         document.getElementById('dayOfWeek-main').textContent = dayOfWeek;
          document.getElementById('day-main').textContent = day;
           document.getElementById('monthNamek-main').textContent = monthName;
		 document.getElementById('fullYear-main').textContent = `${fullYear} год`;					
	/*					
 document.getElementById('date-main').innerHTML = `<p class="date-mainn__number">${day}</p>
       ...
       document.getElementById('date-text').innerHTML = `<p class="info-date__text_position_left">${day}</p>
       <p class="info-date__text_position_center">${monthName} ${fullYear} ГОДА</p>
								<p class="info-date__text_position_right">${dayOfWeek}</p>`;
   ...
   */     
    ...
    // Имена
        document.getElementById('full-names').textContent = `${data.names.groom} & ${data.names.bride}`;
...
*/
        ...
        data.texts.details.forEach((detailText, index) => {
            ...
            detailBlock.innerHTML = `<p class="details__text">${detailText}</p>`; // Добавляем текст детали
